...
module Expr = struct
  ...
  let power (base : [< sql_numeric ] t) (exponent : [< sql_numeric ] t) : [< sql_numeric ] t =
  ...
  let rec to_string : 'a. 'a t -> string =
    ...
    | Unary (operator, right) -> Format.sprintf "%s %s" operator (to_string right)
    | Postary (operator, right) -> Format.sprintf "%s %s" (to_string right) operator
    | Primitive expr -> Primitive.to_string expr
    | Column x -> Column.to_string x
    | Fn (fn, expr) -> Format.sprintf "%s(%s)" fn (to_string expr)
    | Tuple (left, right) -> Format.sprintf "%s, %s" (to_string left) (to_string right)
  ;;
end
...
let rec build_request_tables : 'a. 'a query -> 'a =
  ...
  match query with
  | FROM (_, table) -> table
  | JOIN (from, joined, _) -> build_request_tables from, build_request_tables joined
...
let rec build_request_type : 'a. 'a query -> request =
  ...
  match query with
  | FROM (name, table) -> { name; fields = ExprList.empty (); join = []; where = "" }

    ...
    List.filter_map prev_tables ~f:(fun prev_table ->
      match
        List.find next_tables ~f:(fun next_table ->
          Table.equal_name prev_table next_table && not (Table.equal_fields prev_table next_table))
      with
...
let find_field_operations ~prev ~next =
  ...
  List.fold_left modified ~init:[] ~f:(fun acc (prev_table, next_table) ->
    ...
    let dropped_fields =
      ...
      |> List.map ~f:(fun prev_field -> Operations.drop_column prev_table.name prev_field.name)
    in
...
let find_operations ~prev ~next =
  let creation =
    ...
    let next_tables = next.tables in
    let new_tables =
      List.filter next_tables ~f:(fun t -> not (List.mem prev_tables t ~equal:Table.equal_name))
    in
    List.fold new_tables ~init:[] ~f:(fun acc t ->
      (Operations.create_table t.name :: Operations.add_columns_from_table t) @ acc)
  in
  let drops =
    let prev_tables = prev.tables in
    let next_tables = next.tables in
    let new_tables =
      List.filter prev_tables ~f:(fun t -> not (List.mem next_tables t ~equal:Table.equal_name))
    in
...
let user_table_next =
  Table.make "users" ~fields:[ Field.make "id" ~ty:ty_int; Field.make "name" ~ty:ty_text ]
;;
...
let execute_test ~prev ~next =
  ...
  Fmt.pr "@.Migrations:@.";
  List.iter operations ~f:(fun operation -> Fmt.pr "%s;@." (Operations.to_sql operation))
;;

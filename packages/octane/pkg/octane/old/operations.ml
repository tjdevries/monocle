...
let add_columns_from_table (table : Table.t) =
  List.map table.fields ~f:(fun field -> add_column table.name field.name field.ty)
;;

let create_fields fields =
  let create_field (field : Field.t) = Fmt.str "%s %s" field.name (FieldType.to_sql field.ty) in
...
and alter_table = function
  | AddColumn (table, field, ty) ->
    Fmt.str {|ALTER TABLE "%s" ADD COLUMN "%s" %s|} table field (FieldType.to_sql ty)
  | DropColumn (table, field) -> Fmt.str {|ALTER TABLE "%s" DROP COLUMN "%s"|} table field
;;

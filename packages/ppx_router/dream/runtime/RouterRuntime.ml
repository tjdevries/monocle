open struct
  module IO : RouterRuntimeLib.IO with type 'a t = 'a Lwt.t = struct
    type 'a t = 'a Lwt.t
  ...
  module Request : RouterRuntimeLib.REQUEST with type 'a IO.t = 'a IO.t and type t = Dream.request =
  struct
    module IO = IO
  ...
  module Response :
    ...
    let respond ~status ~headers body = Dream.respond ~status ~headers body
  end

  module Return : RouterRuntimeLib.RETURN with type status = Dream.status and type 'a t = 'a =
  struct
    type status = Dream.status
    type 'a t = 'a
...
include RouterRuntimeLib.Make (Request) (Response) (Return)

...
let pages_handle route _req =
  ...
  | List_users { user_ids } ->
    let ids =
      match user_ids with
      | user_ids ->
        Printf.sprintf "[%s]" (user_ids |> List.map User_id.project |> String.concat ", ")
    in
    Dream.respond (Printf.sprintf "User ids = %s" ids)
  | User_info { user_id } | User_info_via_path { user_id } ->
    Dream.respond (Printf.sprintf "User info for %S" (User_id.project user_id))
  | Signal { level } -> Dream.respond (Printf.sprintf "Signal: %d" (Level.to_int level))
...
let api_handle : type a. a Api.t -> Dream.request -> a RouterRuntime.return Lwt.t =
  fun x _req ->
  match x with
...
let all_handler : Dream.handler =
  let f : type a. a All.t -> Dream.request -> a RouterRuntime.return Lwt.t =
    fun x req ->
    match x with
...
let test () =
  ...
  print_endline (Pages.href (Route_with_implicit_path { param = Some "ok" }));
  print_endline (Pages.href (Hello { name = "world"; modifier = None; greeting = None }));
  print_endline (Pages.href (Hello { name = "world"; modifier = Some Uppercase; greeting = None }));
  print_endline (Api.href (Get_user { id = 121 }));
  print_endline (All.href (Pages (Hello { name = "world"; modifier = None; greeting = None })));
  print_endline (All.href (Api (Get_user { id = 121 })));
  print_endline (Pages.href (User_info { user_id = User_id.inject "username" }));
  print_endline (Pages.href (User_info_via_path { user_id = User_id.inject "username" }));
  print_endline (Pages.href (Signal { level = Warning }));
  print_endline
    (Pages.href (List_users { user_ids = [ User_id.inject "u1"; User_id.inject "u2" ] }));
  ...
  let test_req ?body h method_ target =
    print_endline (Printf.sprintf "## %s %s" (Dream.method_to_string method_) target);
    Lwt_main.run
      (let req = Dream.request ~method_ ~target "" in
       ...
       Dream.body resp
       >>= fun body ->
       print_endline (Printf.sprintf "%s: %s" (Dream.status resp |> Dream.status_to_string) body);
       Lwt.return ())
  in
  ...
  test_req pages_handler `GET "/hello/pct%20encoded?greeting=pct%20encoded";

...
module Pages = struct
  open RouterRuntime.Primitives

  type user_id = User_id.t [@@deriving url_query_via_iso, url_path_via_iso]

  type level = Level.t [@@deriving url_query_via_iso { t = int; inject = of_int; project = to_int }]
...
module Api = struct
  ...
  type _ t =
    ...
    | Raw_response : RouterRuntime.response t [@GET "/raw-response"]
  [@@deriving router]
end

module All = struct
  type _ t =
    | Pages : Pages.t -> RouterRuntime.response t [@prefix "/"]
    | Api : 'a Api.t -> 'a t [@prefix "/nested/api"]
    | Static : { path : string } -> RouterRuntime.response t [@GET "/static/...path"]
  [@@deriving router]
end

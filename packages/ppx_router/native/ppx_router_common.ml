...
let collect_params_rev ~loc uri =
  let rec aux acc = function
    ...
    | x :: xs when String.prefix x ~pre:"..." ->
      if not (List.is_empty xs)
      then Location.raise_errorf ~loc "wildcard pattern can only be at the end of the path";
      let x = String.chop_prefix ~pre:"..." x |> Option.value ~default:"impossible" in
      `wildcard x :: acc
    | "" :: xs -> aux acc xs
    | x :: xs ->
...
let equal_path : path Equal.t =
  let eq_param a b =
    ...
    | _ -> false
  in
  ContainersLabels.Equal.list eq_param
;;

let equal_route_by_path_method : leaf Equal.t =
  fun a b -> ContainersLabels.Equal.poly a.l_method_ b.l_method_ && equal_path a.l_path b.l_path
;;

let equal_route_by_path : leaf Equal.t = fun a b -> equal_path a.l_path b.l_path

let hash_route_by_path : leaf ContainersLabels.Hash.t =
  fun leaf ->
  let open ContainersLabels in
  Hash.list
    (function
      | `param _ -> 0
      | `wildcard _ -> 1
...
let declare_router_attr method_ =
  ...
  in
  method_, Attribute.declare name Attribute.Context.Constructor_declaration pattern (fun x -> x)
;;
...
let attr_prefix =
  ...
  in
  Attribute.declare name Attribute.Context.Constructor_declaration pattern (fun x -> x)
;;
...
let to_supported_arg_type (t : core_type) =
  ...
  | Ptyp_package _ | Ptyp_extension _ ->
    Location.raise_errorf ~loc "cannot automatically derive type parameter decoding/encoding" ()
;;
...
let rec derive_conv suffix t =
  ...
  | `tuple ts ->
    ...
    [%expr [%e evar ~loc name] [%e elist ~loc (List.map ts ~f:(derive_conv suffix))]]
  | `constr (name, args) ->
...
let extract_mount_response = function
  ...
  | Some t ->
    let loc = t.ptyp_loc in
    (match t.ptyp_desc with
     | Ptyp_constr (_, [ [%type: RouterRuntime.response] ]) -> `response
     | Ptyp_constr (_, [ [%type: 'a] ]) -> `passthrough
     | _ -> Location.raise_errorf ~loc "the response type can be either `response` or `'a`")
;;
...
let extract_leaf_response = function
  ...
  | Some t ->
    ...
    (match t.ptyp_desc with
     ...
     | _ ->
       Location.raise_errorf ~loc "the response type can be either `response` or `'a json_response`")
;;
...
let extract td =
  ...
  let param =
    ...
    | _ -> Location.raise_errorf ~loc "only no or a single type parameter is supported"
  in
  let ctors =
    match td.ptype_kind with
    | Ptype_variant ctors -> ctors
    | Ptype_abstract | Ptype_record _ | Ptype_open ->
      Location.raise_errorf ~loc "only variant types are supported for by [@@deriving router]"
  in
  let ctors =
    List.fold_left ctors ~init:[] ~f:(fun ctors ctor ->
      ...
      let kind =
        ...
        | Pcstr_tuple [] -> `leaf []
        | Pcstr_tuple [ { ptyp_desc = Ptyp_constr (t, []); _ } ] -> `mount (t, None)
        | Pcstr_tuple [ { ptyp_desc = Ptyp_constr (t, [ { ptyp_desc = Ptyp_var p; _ } ]); _ } ] ->
          `mount (t, Some p)
        | Pcstr_tuple _ -> Location.raise_errorf ~loc "only record constructors are supported"
      in
      match kind with
      | `mount (m_typ, m_typ_param) ->
        let m_prefix =
          ...
          | Some path ->
            ...
            let path = ContainersLabels.String.split_on_char ~by:'/' path in
            List.filter_map path ~f:(function
              | "" -> None
              | x -> Some x)
        in
        let m_response = extract_mount_response ctor.pcd_res in
        Mount { m_prefix; m_typ; m_typ_param; m_ctor = ctor; m_response } :: ctors
      | `leaf lds ->
        let info =
        ...
        let l_method_, uri =
          ...
          | Some (method_, None) -> method_, Uri.of_string ("/" ^ ctor.pcd_name.txt)
        in
        let lds, l_body =
          ContainersLabels.List.partition_filter_map lds ~f:(fun ld ->
        ...
        let l_body =
          ...
          | [], _ -> None
          | x :: _, `GET ->
            Location.raise_errorf ~loc:x.pld_loc "@body is not allowed for GET requests"
          | [ x ], _ -> Some (x.pld_name.txt, x.pld_type)
          | x :: _, _ ->
            Location.raise_errorf ~loc:x.pld_loc "multiple @body annotations are not allowed"
        in
        let resolve_type name =
          ...
          in
          match typ with
          | None -> Location.raise_errorf ~loc "missing type for param: %S" name
          | Some typ -> typ
        in
          ...
          List.filter_map lds ~f:(fun ld ->
            let is_path_param =
              List.exists l_path ~f:(function
                | `param (name, _) | `wildcard name -> String.equal name ld.pld_name.txt
                | `path _ -> false)
            in
            if is_path_param then None else Some (ld.pld_name.txt, ld.pld_type))
        in
        let l_response = extract_leaf_response ctor.pcd_res in
        let leaf = { l_ctor = ctor; l_method_; l_path; l_query; l_body; l_response } in
        (match
              ...
              [%expr
                Stdlib.List.iter
                  (fun (name, value) ->
                     ...
                     RouterRuntime.Encode.encode_query_key [%e out] name;
                     [%e sep] := '&';
                     Buffer.add_char [%e out] '=';
                     RouterRuntime.Encode.encode_query_value [%e out] value)
                  ([%e derive_conv "to_url_query" typ] [%e estring ~loc name] [%e value])]
            in
            [%expr
              [%e write];
              [%e acc]]
      ...
      let body =
        List.fold_left (List.rev path) ~init:body ~f:(fun acc param ->
          ...
          | `path x ->
            [%expr
              Buffer.add_char [%e out] '/';
              RouterRuntime.Encode.encode_path [%e out] [%e estring ~loc x];
              [%e acc]]
          | `wildcard x ->
            [%expr
          ...
          | `param (x, typ) ->
            ...
            [%expr
              Buffer.add_char [%e out] '/';
              RouterRuntime.Encode.encode_path [%e out] ([%e to_url] [%e evar ~loc x]);
              [%e acc]])
      in
      let body =
        [%expr
  ...
  let derive td (routes : route list) =
    ...
    let cases =
      List.map routes ~f:(function
        | Mount { m_prefix; m_ctor; m_typ; m_typ_param = _; m_response = _ } ->
          let prefix =
            estring
              ~loc
              (match m_prefix with
          ...
          let e =
            [%expr
              [%e prefix]
              ^ [%e evar ~loc (Longident.name (mangle_lid (Suffix "href") m_typ.txt))] [%e x]]
          in
          p --> e
        | Leaf leaf ->
    ...
    in
    [%stri let [%p pvar ~loc name] = [%e we [%expr ([%e pexp_function ~loc cases] : [%t t])]]]
  ;;
end
...
module Derive_method = struct
  let derive td (routes : route list) =
    let loc = td.ptype_loc in
    let name = td.ptype_name.txt in
    let t, we = td_newtype td (Fun.const [%type: [ `GET | `POST | `PUT | `DELETE ]]) in
    ...
    let cases =
      List.map routes ~f:(function
        | Mount m ->
          ...
          let f = evar ~loc (Longident.name (mangle_lid (Suffix "http_method") m.m_typ.txt)) in
          p --> [%expr [%e f] [%e x]]
        | Leaf c ->
...
module Derive_body = struct
  ...
  let derive td routes =
    let loc = td.ptype_loc in
    let name = td.ptype_name.txt in
    let t, we = td_newtype td (Fun.const [%type: RouterRuntime.json option]) in
  ...
  let derive td routes =
    ...
    let t, we =
      td_newtype td (fun p ->
        let a =
          match p with
          | None -> [%type: RouterRuntime.response]
          | Some p -> ptyp_constr ~loc { loc; txt = Lident p } []
        in
        [%type: [%t a] RouterRuntime.Witness.t])
    in
    let name = mangle name in
    let cases =
      List.map routes ~f:(function
    ...
    let bnds =
      List.filter_map routes ~f:(function
        ...
        | Leaf c ->
          ...
          let expr = [%expr RouterRuntime.Witness.create ()] in
          let t =
            match c.l_response with
            | `response -> [%type: RouterRuntime.response]
            | `json_response t -> t
          in
          Some [%stri let [%p pat] : [%t t] RouterRuntime.Witness.t = [%e expr]])
    in
    [ [%stri open [%m pmod_structure ~loc bnds]]; [%stri let [%p pvar ~loc name] = [%e e]] ]
  ;;
end
...
module Derive_url_query_via_json = struct
  let derive ~ctxt:_ td =
    ...
    [%str
      let [%p to_p] = fun k v -> [ k, Ppx_deriving_json_runtime.to_string ([%e to_json] v) ]
      ...
      let [%p of_p] =
        ...
        | Some v ->
          ...
          (try Stdlib.Result.Ok ([%e of_json] json) with
           | _ -> Stdlib.Result.Error ("error parsing query param: " ^ k))
      ;;]
  ;;

  let register () =
    let derive ~ctxt (_rec_flag, tds) = ContainersLabels.List.flat_map tds ~f:(derive ~ctxt) in
...
module Derive_url_query_via_iso = struct
  ...
  let assert_simple_lid ~default x =
    ContainersLabels.Option.map
      (fun { loc; txt } ->
         match txt with
         | Lident n -> n
         | _ -> Location.raise_errorf ~loc "must be a simple identifier (not longident)")
      x
  ...
  let gen_iso ~loc td (via, inject, project) =
    let base =
      ...
      | Ptype_abstract, Some { ptyp_desc = Ptyp_constr (lid, []); _ } -> lid.txt
      | _ -> Location.raise_errorf ~loc "should be a type alias with no parameters"
    in
  ...
  let derive ~ctxt:_ iso td =
    ...
    [%str
      ...
      let [%p of_p] = fun k xs -> Stdlib.Result.map [%e inject] ([%e of_] k xs)]
  ;;

  let register () =
    let derive ~ctxt (_rec_flag, tds) via inject project =
      ContainersLabels.List.flat_map tds ~f:(derive ~ctxt (via, inject, project))
    in
    let args =
      Deriving.Args.(
        empty
...
let register ~derive () =
  let derive_router ~ctxt (_rec_flag, type_decls) =
    ...
    | [ td ] -> derive td
    | [] -> assert false
    | _ -> Location.raise_errorf ~loc "expected exactly one type declaration"
  in
  let args = Deriving.Args.(empty) in

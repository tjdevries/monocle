  ...
  let derive td routes =
    ...
    let t, we =
      td_newtype td (fun typ ->
        ...
        in
        [%type: [%t value_typ] -> RouterRuntime.json])
    in
      ...
      List.map routes ~f:(function
        ...
        | Leaf c ->
          ...
          let e =
            ...
            | `response -> [%expr failwith "response cannot be serialized to json"]
            | `json_response typ -> [%expr [%to_json: [%t typ]] _value]
          in
          p --> e)
    in
    let e = we [%expr (fun route _value -> [%e pexp_match ~loc [%expr route] cases] : [%t t])] in
  ...
  | Some param ->
    ...
    ptyp_poly
      ...
      [%type:
        ...
        -> RouterRuntime.request
        -> [%t param] RouterRuntime.return RouterRuntime.IO.t]
  | None ->
    [%type:
      [%t td_to_ty param td]
      -> RouterRuntime.request
      -> RouterRuntime.response RouterRuntime.return RouterRuntime.IO.t]
;;
...
let td_to_ty_enc param td =
  ...
  let result =
    ...
    | Some param -> param
  in
  [%type: [%t result] RouterRuntime.Handle.encode]
;;
...
let derive_mount td m =
  ...
  let expr =
    let routes = evar ~loc (Longident.name (routes_name_lid m.m_typ.txt)) in
    let p inner =
      ppat_construct ~loc { loc; txt = packed_ctor_name_lid m.m_typ.txt } (Some inner)
    in
    let make = pexp_construct ~loc { loc; txt = Lident m.m_ctor.pcd_name.txt } (Some [%expr x]) in
    ...
    let encode =
      ...
      | `response -> [%expr RouterRuntime.Handle.Encode_raw]
    in
    [%expr
      Stdlib.List.map
        (fun route ->
           let f f req =
             RouterRuntime.IO.bind (f req) (fun [%p p [%pat? x, _encode]] ->
               RouterRuntime.IO.return [%e make_with_encode encode])
           in
           RouterRuntime.Handle.prefix_route
    ...
    | init :: params ->
      let has_wildcard, body =
        let f = function
          | `param (name, ty) ->
            ...
            let to_url =
              [%expr
                fun x ->
                  ...
                  RouterRuntime.Encode.encode_path buf x;
                  Buffer.contents buf]
            in
            ...
            let of_url =
              [%expr
                fun x ->
                  let x = RouterRuntime.Decode.decode_path x in
                  [%e of_url] x]
            in
            false, [%expr Routes.pattern [%e to_url] [%e of_url] [%e estring ~loc name]]
        ...
        List.fold_left params ~init:(f init) ~f:(fun (_, body) param ->
          ...
          let op = if has_wildcard then [%expr Routes.( /? )] else [%expr Routes.( / )] in
          has_wildcard, [%expr [%e op] [%e body] [%e eparam]])
      in
      if has_wildcard then body else [%expr Routes.( /? ) [%e body] Routes.nil]
  in
  let make =
    let params =
    ...
    let by_method =
      let init = [ ppat_any ~loc --> [%expr raise RouterRuntime.Handle.Method_not_allowed] ] in
      ContainersLabels.List.fold_left ctors ~init ~f:(fun cases leaf ->
        ...
        let args =
          List.map2 path_params params ~f:(fun name value ->
            ...
            | `wildcard name ->
              { loc; txt = Lident name }, [%expr Routes.Parts.wildcard_match [%e v]])
        in
        let args =
          args
          @ List.filter_map leaf.l_query ~f:(fun (name, typ) ->
            ...
            let value =
              [%expr
                match [%e of_url] [%e estring ~loc name] __url_query with
                | Ok v -> v
                | Error err ->
                  raise (RouterRuntime.Handle.Invalid_query_parameter ([%e estring ~loc name], err))]
            in
        ...
        let to_response =
          ...
          | `response -> [%expr RouterRuntime.Handle.Encode_raw]
          | `json_response t -> [%expr RouterRuntime.Handle.Encode_json [%to_json: [%t t]]]
        in
        let make args =
          let args =
            match args with
        ...
        let expr =
          ...
          | None -> [%expr RouterRuntime.IO.return [%e make args]]
          | Some (name, body) ->
            let name = { loc; txt = Lident name } in
            let args = (name, ebody) :: args in
            [%expr
              RouterRuntime.IO.bind
                (RouterRuntime.Request.body [%e req])
                (fun [%p pbody] ->
                   let [%p pbody] =
                     try Yojson.Basic.from_string [%e ebody] with
                     | Yojson.Json_error msg -> raise (RouterRuntime.Handle.Invalid_body msg)
                   in
                   let [%p pbody] =
                     try [%of_json: [%t body]] [%e ebody] with
                     | Yojson.Basic.Util.Type_error (msg, _) ->
                       raise (RouterRuntime.Handle.Invalid_body msg)
                   in
                   RouterRuntime.IO.return [%e make args])]
        in
        let expr =
          [%expr
            let __url_query = RouterRuntime.Request.queries [%e req] in
            [%e expr]]
        in
        (pat --> expr) :: cases)
    in
    let make =
      [%expr
        fun ([%p preq] : RouterRuntime.request) ->
          [%e pexp_match ~loc [%expr RouterRuntime.Request.method_ [%e req]] by_method]]
    in
    List.fold_left (List.rev params) ~init:make ~f:(fun body param ->
      pexp_fun ~loc Nolabel None (pvar ~loc param) body)
  in
  let pat = pvar ~loc name in
  let expr = [%expr [ RouterRuntime.Handle.Route ([%e body], [%e make], Stdlib.Fun.id) ]] in
  value_binding ~loc ~pat ~expr
;;
...
let derive_router_td td =
  ...
  in
  let loc = td.ptype_loc in
  let packed_ty = ptyp_constr ~loc { loc; txt = Lident (packed_name td) } [] in
  let packed_stri =
    pstr_type
      ~loc
      Recursive
          ...
          ~kind:
            (Ptype_variant
               [ { pcd_name = { txt = packed_ctor_name td; loc }
                 ; pcd_args = Pcstr_tuple [ td_to_ty param td; td_to_ty_enc param td ]
  ...
  ; packed_stri
  ; handler_stri
  ; [%stri let [%p pvar ~loc (routes_name td)] = [%e derive_routes td leafs mounts]]
  ; [%stri
      let [%p pvar ~loc (handle_name td)] =
        let router =
          RouterRuntime.Handle.make
            (let routes =
               Stdlib.List.map RouterRuntime.Handle.to_route [%e evar ~loc (routes_name td)]
             in
             Routes.one_of routes)
        in
        fun ({ f } : [%t ptyp_constr ~loc { loc; txt = Lident (handler_name td) } []]) ->
          RouterRuntime.Handle.handle
            router
            (fun
                [%p
                  ppat_construct
                    ...
                    (Some [%pat? p, encode])]
                 req
               -> RouterRuntime.IO.bind (f p req) (RouterRuntime.Handle.encode encode))
      ;;]
  ; [%stri
      let [%p pvar ~loc (handle_name td)] =
        [%e

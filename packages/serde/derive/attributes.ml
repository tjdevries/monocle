...
type field_attributes =
  ...
  ; should_skip :
      [ `skip_serializing_if of string | `skip_deserializing_if of string | `always | `never ]
  }
...
let of_record_attributes attributes =
  ...
  (* Retrieve fields *)
  let serde_attr = List.find_opt (fun attr -> attr.attr_name.txt = "serde") attributes in
  Option.iter
    (fun attr ->
       ...
       | PStr [ { pstr_desc = Pstr_eval ({ pexp_desc = Pexp_record (fields, _); _ }, _); _ } ] ->
         List.iter
           (function
             | { txt = Lident "tag"; _ }, { pexp_desc = Pexp_constant (Pconst_string (s, _, _)); _ }
             ...
             | ( { txt = Lident "content"; _ }
               ...
               let new_mode =
                 ...
                 | _ ->
                   failwith
                     "[ppx_serde] You can only use the 'content' attribute after the 'tag' attribute."
               in
               variant_tagging_mode := new_mode
             | { txt = Lident "untagged"; _ }, [%expr true] -> variant_tagging_mode := `untagged
  ...
  let name =
    ref
      (match type_attributes.rename_all with
       ...
       | Some `SCREAMING_KEBAB_CASE -> kebab_case lbl.pld_name.txt |> String.uppercase_ascii

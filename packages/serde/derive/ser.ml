...
let rec serializer_for_type ~ctxt (core_type : Parsetree.core_type) =
  ...
  | Ptyp_constr (name, _args) ->
    let ser_fn = serializer_fn_name_for_longident name |> var ~ctxt |> Ast.pexp_ident ~loc in
    [%expr s [%e ser_fn]]
  | Ptyp_any | Ptyp_var _
  | Ptyp_arrow (_, _, _)
  | Ptyp_tuple _
...
let gen_serialize_externally_tagged_variant_impl ~ctxt ptype_name type_attributes cstr_declarations =
  let loc = loc ~ctxt in
  ...
  let pattern_of_constructor cstr =
    ...
    | Pcstr_tuple parts ->
      ...
      Some (Ast.ppat_tuple ~loc (List.map (fun _ -> Ast.pvar ~loc (gensym ~ctxt).txt) parts))
  ...
  let ser_by_constructor type_name idx cstr =
    ...
    match cstr.pcd_args with
    | Pcstr_tuple [] -> [%expr unit_variant ctx [%e type_name] [%e idx] [%e name]]
    | Pcstr_tuple [ arg ] ->
    ...
    | Pcstr_tuple args ->
      ...
      in
      [%expr
        tuple_variant ctx [%e type_name] [%e idx] [%e name] [%e arg_count] (fun ctx -> [%e calls])]
    | Pcstr_record labels ->
      ...
      [%expr
        record_variant ctx [%e type_name] [%e idx] [%e name] [%e field_count] (fun ctx ->
          [%e fields])]
  in
    ...
    | Pcstr_tuple [ arg ] ->
      ...
      [%expr
        record ctx "" 2 (fun ctx ->
          let* () = field ctx [%e Ast.estring ~loc tag_field_name] (string [%e name]) in
          field ctx [%e Ast.estring ~loc content_field_name] (fun ctx -> [%e ser]))]
    | Pcstr_tuple args ->
      let arg_count = Ast.eint ~loc (List.length args) in
      let gensym = gensym () in
      let calls =
...
let gen_serialize_variant_impl ~ctxt ptype_name type_attributes cstr_declarations =
  match type_attributes.Attributes.variant_tagging_mode with
  | `externally_tagged ->
    gen_serialize_externally_tagged_variant_impl ~ctxt ptype_name type_attributes cstr_declarations
...
let gen_serialize_record_impl ~ctxt ptype_name type_attributes label_declarations =
  let loc = loc ~ctxt in
  let type_name = Ast.estring ~loc ptype_name.txt in
...
let gen_serialize_impl ~ctxt type_decl =
  ...
  let body =
    ...
    | { ptype_kind = Ptype_record label_declarations; ptype_name; _ } ->
      gen_serialize_record_impl ~ctxt ptype_name type_attributes label_declarations
    | { ptype_kind = Ptype_variant cstrs_declaration; ptype_name; _ } ->
      gen_serialize_variant_impl ~ctxt ptype_name type_attributes cstrs_declaration
...
let register = Deriving.add "serialize" ~str_type_decl:impl_generator ~sig_type_decl:intf_generator

...
[@@deriving serialize, deserialize] [@@serde { rename_all = "SCREAMING-KEBAB-CASE" }]
...
let test_de_with_screamingkebabcase () =
  ...
  | Ok x ->
    assert (x.kebab_case = true);
    Format.printf "serde_json.ser/de test \"parsed: with_screamingkebabcase\"@."
  | Error _ -> failwith "Could not parse: with_screamingkebabcase"
;;

type with_screamingsnakecase = { snake_case : bool }
[@@deriving serialize, deserialize] [@@serde { rename_all = "SCREAMING_SNAKE_CASE" }]
...
let test_de_with_screamingsnakecase () =
  ...
  | Ok x ->
    ...
    Format.printf "serde_json.ser/de test \"parsed: with_screamingsnakecase\"@."

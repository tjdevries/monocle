...
let _serde_json_roundtrip_tests =
  let test str pp ser de value expect_str =
    ...
    then Format.printf "serde_json.ser/de test %S %s\r\n%!" str (keyword "OK")
    else (
      Format.printf
        "%s\n\nExpected:\n\n%s\n\nbut found:\n\n%s\n\n"
  ...
  test
    ...
    pp_variant
    Ser.(serializer @@ fun A ctx -> unit_variant ctx "simple_variant" 0 "Wrong_variant_name")
    De.(
      deserializer
      @@ fun ctx ->
      let field_visitor =
  ...
  test
    ...
    pp_variant_with_arg
    Ser.(
      serializer @@ fun (B i) ctx -> newtype_variant ctx "simple_variant" 0 "Wrong_variant" (int i))
    De.(
      deserializer
      @@ fun ctx ->
      let field_visitor =
  ...
  test
    "record_with_key_rename"
    pp_with_type_field
    serialize_with_type_field
    deserialize_with_type_field
    { type_ = "hello" }
    {|{type="hello"}|};
...
let _serde_json_parse_test_no_key =
  ...
  match parsed with
  | Ok _ -> Format.printf "serde_json.ser/de test %S %s\r\n%!" "parsed with no key" (keyword "OK")
  | Error _ ->
    Format.printf "serde_json.ser/de test %S %s\r\n%!" "parsed with no key" (error "Failed!");
    assert false
;;
...
let _serde_json_parse_test_with_default =
  ...
  Format.printf "serde_json.ser/de test %S %s\r\n%!" "parsed with default" (keyword "OK");
  ()
;;

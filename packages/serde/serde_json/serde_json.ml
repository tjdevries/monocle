...
module Json = struct
  module Parser = struct
    ...
    let of_string string = { yojson = Yojson.init_lexer (); lexbuf = Lexing.from_string string }

    let _run fn = Ok (fn ())

    let peek { lexbuf; _ } =
      if lexbuf.lex_curr_pos < lexbuf.lex_buffer_len
      then Some (Bytes.unsafe_to_string lexbuf.lex_buffer).[lexbuf.lex_curr_pos]
      else None
    ;;

    let read_bool { yojson; lexbuf } = _run (fun () -> Yojson.Safe.read_bool yojson lexbuf)

    let read_string { yojson; lexbuf } = _run (fun () -> Yojson.Safe.read_string yojson lexbuf)

    let read_int8 { yojson; lexbuf } = _run (fun () -> Yojson.Safe.read_int8 yojson lexbuf)

    let read_int { yojson; lexbuf } = _run (fun () -> Yojson.Safe.read_int yojson lexbuf)

    let read_int32 { yojson; lexbuf } = _run (fun () -> Yojson.Safe.read_int32 yojson lexbuf)

    let read_int64 { yojson; lexbuf } = _run (fun () -> Yojson.Safe.read_int64 yojson lexbuf)

    let read_float { yojson; lexbuf } = _run (fun () -> Yojson.Safe.read_number yojson lexbuf)

    let read_null_if_possible { yojson; lexbuf } =
      _run (fun () -> Yojson.Safe.read_null_if_possible yojson lexbuf)
    ;;

    let read_null { yojson; lexbuf } = _run (fun () -> Yojson.Safe.read_null yojson lexbuf)

    let read_object_start { yojson; lexbuf } = _run (fun () -> Yojson.Safe.read_lcurl yojson lexbuf)
    ...
    let read_open_bracket { yojson; lexbuf } = _run (fun () -> Yojson.Safe.read_lbr yojson lexbuf)

    let read_close_bracket { yojson; lexbuf } = _run (fun () -> Yojson.Safe.read_rbr yojson lexbuf)

    let read_comma { yojson; lexbuf } = _run (fun () -> Yojson.Safe.read_comma yojson lexbuf)

    let read_colon { yojson; lexbuf } = _run (fun () -> Yojson.Safe.read_colon yojson lexbuf)
...
module Serializer = struct
  ...
  let serialize_bool _self (S { fmt; _ }) bool = Rio.write_all fmt ~buf:(Bool.to_string bool)

  let serialize_string _self (S { fmt; _ }) string =
    Rio.write_all fmt ~buf:(Format.sprintf "%S" string)
  ;;

  let serialize_int8 _self (S { fmt; _ }) int = Rio.write_all fmt ~buf:(String.make 1 int)

  let serialize_int16 _self (S { fmt; _ }) int = Rio.write_all fmt ~buf:(Int.to_string int)

  let serialize_int31 _self (S { fmt; _ }) int = Rio.write_all fmt ~buf:(Int.to_string int)

  let serialize_int32 _self (S { fmt; _ }) int = Rio.write_all fmt ~buf:(Int32.to_string int)

  let serialize_int64 _self (S { fmt; _ }) int = Rio.write_all fmt ~buf:(Int64.to_string int)

  let serialize_float _self (S { fmt; _ }) float = Rio.write_all fmt ~buf:(Float.to_string float)
  ...
  let serialize_unit_variant _self (S { fmt; _ }) ~var_type:_ ~cstr_idx:_ ~cstr_name =
    Rio.write_all fmt ~buf:(Format.sprintf "%S" cstr_name)
  ;;

  let serialize_tuple_variant self (S { fmt; _ }) ~var_type:_ ~cstr_idx:_ ~cstr_name ~size values =
  ...
  let serialize_record_variant self (S { fmt; _ }) ~var_type:_ ~cstr_idx:_ ~cstr_name ~size values =
    let* () = Fmt.begin_object fmt in
    let* () = Fmt.begin_object_key ~first:true fmt in
    let* () = Rio.write_all fmt ~buf:(Format.sprintf "%S" cstr_name) in
    let* () = Fmt.end_object_key fmt in
  ...
  let serialize_newtype_variant self (S { fmt; _ }) ~var_type:_ ~cstr_idx:_ ~cstr_name field =
    let* () = Fmt.begin_object fmt in
    let* () = Fmt.begin_object_key ~first:true fmt in
    let* () = Rio.write_all fmt ~buf:(Format.sprintf "%S" cstr_name) in
    let* () = Fmt.end_object_key fmt in

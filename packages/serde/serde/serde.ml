...
let pp_list pp_el fmt t =
  Format.fprintf fmt "[";
  Format.pp_print_list ~pp_sep:(fun fmt () -> Format.fprintf fmt "; ") pp_el fmt t;
  Format.fprintf fmt "]"
;;
...
module rec Ser_base : sig
  ...
  and ('value, 'state, 'output) ctx =
    | Ctx of ('value, 'state, 'output) t * ('state, 'output) Ser_base.serializer * 'state
...
module Ser = struct
  ...
  let list ser values ctx =
    sequence ctx (List.length values)
    @@ fun ctx ->
    List.fold_left
      (fun acc el ->
         match acc with
         | Ok () -> element ctx (ser el)
         | _ -> acc)
      (Ok ())
      values
  ;;
  ...
  let s
    : type value value2 state output.
      (value, state, output) t -> value -> (value2, state, output) ctx -> (output, error) result
    =
...
module rec De_base : sig
  ...
  module type Deserializer = sig
    ...
    val deserialize_element : ctx -> state -> 'value t -> ('value option, error) result
    ...
    val deserialize_newtype_variant : ctx -> state -> 'value t -> ('value, error) result
    ...
    val deserialize_field : ctx -> state -> name:string -> 'value t -> ('value, error) result

    val deserialize_key : ctx -> state -> ('value, 'tag) visitor -> ('value option, error) result

    val deserialize_identifier : ctx -> state -> ('value, 'tag) visitor -> ('value, error) result
    ...
    val deserialize_option : ctx -> state -> 'value t -> ('value option, error) result
...
module Visitor = struct
  ...
  let make ?(visit_int = default.visit_int) ?(visit_string = default.visit_string) () =
...
module De = struct
  ...
  let deserialize_int8 (Ctx ((module D), state) as ctx : ctx) = D.deserialize_int8 ctx state

  let deserialize_int16 (Ctx ((module D), state) as ctx : ctx) = D.deserialize_int16 ctx state

  let deserialize_int31 (Ctx ((module D), state) as ctx : ctx) = D.deserialize_int31 ctx state

  let deserialize_int32 (Ctx ((module D), state) as ctx : ctx) = D.deserialize_int32 ctx state

  let deserialize_int64 (Ctx ((module D), state) as ctx : ctx) = D.deserialize_int64 ctx state

  let deserialize_bool (Ctx ((module D), state) as ctx : ctx) = D.deserialize_bool ctx state

  let deserialize_float (Ctx ((module D), state) as ctx : ctx) = D.deserialize_float ctx state
  ...
  let deserialize_variant (Ctx ((module D), state) as ctx : ctx) ~de ~name ~variants =
  ...
  let deserialize_string (Ctx ((module D), state) as ctx : ctx) = D.deserialize_string ctx state
...
let deserialize
  : type value state output. state Deserializer.t -> state -> value De.t -> (value, error) result
  =

...
module Serde_bin = struct
  module Serializer = struct
    ...
    let serialize_tuple_variant self (S _) ~var_type:_ ~cstr_idx ~cstr_name:_ ~size args =
      let* () = Ser.serialize_int31 cstr_idx self in
      let* () = Ser.serialize_int31 size self in
      Ser.serialize self args
    ;;

    let serialize_newtype_variant self (S _) ~var_type:_ ~cstr_idx ~cstr_name:_ args =
      let* () = Ser.serialize_int31 cstr_idx self in
      Ser.serialize self args
    ;;

    let serialize_record_variant self (S _) ~var_type:_ ~cstr_idx ~cstr_name:_ ~size args =
      let* () = Ser.serialize_int31 cstr_idx self in
      let* () = Ser.serialize_int31 size self in
      Ser.serialize self args
    ;;

    let serialize_record self (S _) ~rec_type:_ ~size:_ fields = Ser.serialize self fields
  ...
  module Deserializer = struct
    ...
    let deserialize_record self _state ~name:_ ~size:_ fields = De.deserialize self fields

    let deserialize_field self _state ~name:_ element = De.deserialize self element
    ...
    let deserialize_variant self _state de ~name:_ ~variants:_ = De.deserialize self de

    (* TODO: Could probably test this but I don't think it's worth it? *)
    let deserialize_ignored_any _ _ = failwith "Should not be called in this test"
  end
  ...
  let of_string de string =
    ...
    in
    let reader = Rio.Reader.of_read_src (module StrRead) StrRead.(make string) in
...
let pp_record fmt { name; year } = Format.fprintf fmt "{name=%S;year=%d}" name year
...
let _serde_bin_roundtrip_tests =
  let test str pp ser de value expect_str =
    ...
    if String.equal actual_str expect_str
    then Format.printf "serde_bin.ser/de test %S %s\r\n%!" str (keyword "OK")
    else (
      Format.printf
        "%s\n\nExpected:\n\n%s\n\nbut found:\n\n%s\n\n"
  ...
  test
    ...
    De.(
      ...
      let field_visitor =
        Visitor.
          { default with
            visit_int = (fun _ctx int -> if int = 0 then Ok `A else Error `invalid_tag)
          }
      in

      variant ctx "simple_variant" [ "A" ]
      @@ fun ctx ->
  ...
  test
    "variant_without_args and wrong serialization"
    pp_variant
    Ser.(serializer @@ fun A ctx -> unit_variant ctx "simple_variant" 199 "Wrong_variant_name")
    De.(
      deserializer
      @@ fun ctx ->
      let field_visitor =
  ...
  test
    "variant with one arg"
    pp_variant_with_arg
    Ser.(serializer @@ fun (B i) ctx -> newtype_variant ctx "simple_variant" 0 "B" (int i))
    De.(
      deserializer
      @@ fun ctx ->
      let field_visitor =
        Visitor.
          { default with
            visit_int = (fun _ctx int -> if int = 0 then Ok `B else Error `invalid_tag)
          }
      in

      variant ctx "simple_variant" [ "B" ]
      @@ fun ctx ->
  ...
  test
    "variant with one arg and wrong serialization"
    pp_variant_with_arg
    Ser.(
      serializer
      @@ fun (B i) ctx -> newtype_variant ctx "simple_variant" 990 "Wrong_variant" (int i))
    De.(
      deserializer
      @@ fun ctx ->
      let field_visitor =
  ...
  test
    "variant with many args"
    pp_variant_with_many_arg
    Ser.(
      serializer
      @@ fun (C (i, str, flt)) ctx ->
      tuple_variant ctx "variant_with_many_args" 0 "C" 3
      @@ fun ctx ->
    ...
    De.(
      ...
      let field_visitor =
        Visitor.
          { default with
            visit_int = (fun _ctx int -> if int = 0 then Ok `C else Error `invalid_tag)
          }
      in

      variant ctx "variant_with_many_args" [ "C" ]
      @@ fun ctx ->
  ...
  test
    ...
    pp_variant_with_inline_record
    Ser.(
      serializer
      @@ fun (D { is_inline }) ctx ->
      record_variant ctx "variant_with_many_args" 0 "C" 2
      @@ fun ctx ->
    ...
    De.(
      ...
      let field_visitor =
        Visitor.
          { default with
            visit_int = (fun _ctx int -> if int = 0 then Ok `D else Error `invalid_tag)
          }
      in
  ...
  test
    "record_with_nested_records"
    pp_record_nested
    Ser.(
      let nested_serializer =
        serializer
        @@ fun nr ctx ->
        record ctx "nested" 1 @@ fun ctx -> field ctx "nested_flag" (bool nr.nested_flag)
      in
      serializer
      @@ fun r ctx ->
      record ctx "record_nested" 1 @@ fun ctx -> field ctx "nested" (s nested_serializer r.nested))
    De.(
      let nested_deserializer =
        deserializer
        @@ fun ctx ->
        record ctx "record_nested" 1
        @@ fun ctx ->
  ...
  let option_serializer = Ser.(serializer @@ fun opt ctx -> option string opt ctx) in
  ...
  test "option/none" pp_with_option option_serializer option_deserializer None {|None|};

  let list_serializer =
    Ser.(
      serializer
      @@ fun ls ctx ->
      sequence ctx (List.length ls)
      @@ fun ctx ->
      List.fold_left
        (fun acc el ->
           match acc with
           | Ok () -> element ctx (string el)
           | _ -> acc)
        (Ok ())
        ls)
  in
  ...
  let list_deserializer =
    De.(
      ...
      read_elements size [])
  in
  test "list/empty" pp_with_list list_serializer list_deserializer ([] : with_list) {|[]|};
  ...
  in
  test "array/empty" pp_with_array array_serializer array_deserializer ([||] : with_array) {|[||]|};
  ...
  let nested_opt_ser =
    Ser.(
      serializer
      @@ fun r ctx ->
      record ctx "with_nested_option" 1
      @@ fun ctx -> field ctx "nested_opt" (s option_serializer r.nested_opt))
  in
  let nested_opt_de =
    De.(
      deserializer
  ...
  in
  test
    "record with nested option/none"
    pp_with_nested_option
    nested_opt_ser
    nested_opt_de
    { nested_opt = None }
    {|({nested_opt=None})|};
  test
    "record with nested option/some"
    pp_with_nested_option
    nested_opt_ser
    nested_opt_de
